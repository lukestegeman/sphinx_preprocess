...
parser.add_argument("--OutputList", type=str, default='',
                    help="Optional file to write paths of successfully modified json files")
...
def update_jsons(json_list, newpath, output_list_fh=None):
    ...
    for json_fname in json_list:
        ...
        # TRY TO READ THE FILE INTO A JSON OBJECT AND UPDATE IT
        try:
            ...
            # WRITE THE OUTPUT PATH TO THE OUTPUT LIST IF PROVIDED
            if output_list_fh is not None:
                output_list_fh.write(newfname + '\n')
...
if model_list:
    ...
    if args.OutputList:
        with open(args.OutputList, 'w') as output_list_fh:
            update_jsons(model_list, args.newpath, output_list_fh)
    else:
